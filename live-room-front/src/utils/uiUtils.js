...
class UiUtils extends Utils {
  ...
  getUserManger(code) {
    return store.state.default.userInfo &&
      store.state.default.userInfo.managerList
      ? store.state.default.userInfo.managerList.find(
          r => r.roleCode === code
        ) || {}
      : {};
  }
  getUserInfo(key) {
    return store.state.default.userInfo
      ? store.state.default.userInfo[key]
      : "";
  }
  ...
  getAreaCode(name) {
    ...
    const area = store.state.default.allAreas.find(
      a => a.provinceName === name
    );
    return area ? area.id : "";
  }
  showLoading() {
